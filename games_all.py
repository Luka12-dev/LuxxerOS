...
class GameLauncherWidget:
    """
    A QWidget-like class (actually returns a QWidget instance from create_widget)
    so it can be embedded inside Luxxer_OS. Use create_widget() to get actual QWidget.
    """
    def __init__(self):
...
# Use this class in APP_MAPPING. It returns a QWidget that Luxxer_MDI can host.
from PyQt6.QtWidgets import QWidget as _QWidget
class GamesApp(_QWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        # build the launcher UI and embed it into this widget
        try:
...
if __name__ == "__main__":
    # if invoked with a game key, run that game (this is how launcher spawns them)
    if len(sys.argv) > 1:
        run_game_by_name(sys.argv[1])
    else:
        launcher_main()