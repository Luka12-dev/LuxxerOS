...
class ReaderModeBrowserApp(WebBrowserApp):
    ...
    def __init__(self):
        ...
        # Napravi dugme za reader mode direktno u glavnom layout-u
        self.btn_reader = QPushButton("Reader Mode")
        self.layout.insertWidget(0, self.btn_reader)  # dodaj na vrh layout-a
    ...
    def _apply_reader(self):
        if WEBENGINE_AVAILABLE:
            try:
                # Primeni reader mode na trenutno aktivni tab
                current_tab = self.tabs.currentWidget()
                if current_tab and hasattr(current_tab, "view"):
                    current_tab.view.page().runJavaScript(self.JS_READER)
