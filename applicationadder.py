...
class ExternalAppWidget(QWidget):
    """
    Simple widget that allows launching/stopping an external command in a subprocess.
    It's safe to register this class in APP_MAPPING as the factory that returns this QWidget.
    """
...
def register_app_globally(entry: dict):
    """
    entry: {'name': str, 'cmd': str, 'icon': optional str, 'placements': list e.g. ['dock','start','desktop']}
    Attempts to:
      - add to Luxxer_OS.APP_MAPPING[name] = lambda: ExternalAppWidget(cmd)
      - add small button to main_win.dock if available (best-effort; safe)
      - add to main_win.start_menu if available (best-effort)
    """
    try:
        import Luxxer_OS
    except Exception:
...
class ApplicationAdder(QMainWindow):
    """
    Full-featured app to add/remove user apps.
    Fields: Name, Command, Icon (optional), placements (dock/start/desktop)
    """
